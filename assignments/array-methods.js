...
let fullName = [];
runners.forEach(bothNames);

function bothNames(runners) {
...
console.table(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 find people who donated less than $100 so that you can pressure them later.
let skinFlints = runners.filter(function(money) {
  return money.donation < 100;
});
console.log(skinFlints);

// Problem 2 make a list of who needs XL shirts.

const result = runners.filter(runner => runner.shirt_size === "XL");

console.log(result);

// Problem 3 return all the runner companies and names of each runner as own object into an array.

const runnerInfo = [];
runners.forEach(runner => {
  console.log(runner);
  runnerInfo.push({
    newCompany: runner.company_name,
    newFirstName: runner.first_name
  });
});
